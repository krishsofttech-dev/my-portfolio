...
var _db   = null;  // Realtime Database reference
var _auth = null;

function onFirebaseReady() {
  firebase.initializeApp(firebaseConfig);
  _auth = firebase.auth();
  _db   = firebase.database();  // ← Realtime Database
...
function dbSet(key, value) {
  if (!_db) return;
  _db.ref('siteData/' + key).set(value)
    .catch(function(e) { console.error('DB write error:', e); });
}

function loadAllFromDB() {

  if (!_db) return;
  _db.ref('siteData').get()
    .then(function(snapshot) {
    ...
    .catch(function(e) {
      ...
      // Fall back to defaults so site still renders
      stats=DEFAULT_STATS; skills=DEFAULT_SKILLS; projects=DEFAULT_PROJECTS;
      experience=DEFAULT_EXPERIENCE; education=DEFAULT_EDUCATION;
      contact=DEFAULT_CONTACT; profile=Object.assign({},DEFAULT_PROFILE);
      renderStats(); renderSkills(); renderProjects();
      renderExperience(); renderEducation(); renderContact();
      applyProfileLinks(); startTypewriter();
    });
}
...
function saveLinksForm(){
  ...
  dbSet('profile',profile);   // ← Realtime DB save
...
function saveProjectForm(){
  ...
  dbSet('projects',projects);   // ← Realtime DB save
...
function saveStatForm(idx){stats[idx].label=document.getElementById('st-label').value.trim();stats[idx].value=parseInt(document.getElementById('st-value').value)||0;stats[idx].suffix=document.getElementById('st-suffix').value;dbSet('stats',stats);renderStats();activeSect='stats';openAdmin();showToast('Stat saved');}
...
function deleteItem(type,id){
  ...
  if(type==='projects'){projects=map[type];dbSet('projects',projects);renderProjects();}
  else if(type==='skills'){skills=map[type];dbSet('skills',skills);renderSkills();}
  else if(type==='experience'){experience=map[type];dbSet('experience',experience);renderExperience();}
  else if(type==='education'){education=map[type];dbSet('education',education);renderEducation();}
  else if(type==='contact'){contact=map[type];dbSet('contact',contact);renderContact();}
...
function saveJWT(){var v=document.getElementById('jwt-in').value.trim();pinataJWT=v;dbSet('pinataJWT',v);updatePinataLabel();cm('pin');showToast(v?'Pinata JWT saved':'JWT cleared');}
