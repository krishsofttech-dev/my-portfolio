...
async function saveSiteData() {
  ...
  var data = {
    stats: stats, skills: skills, projects: projects,
  ...
  try {
    ...
    var res = await fetch(PINATA_API + '/pinning/pinFileToIPFS', {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + pinataJWT },
      body: fd
    });
...
function applyData(data) {
  ...
  profile    = data.profile    || { githubUrl:'', linkedinUrl:'', cvIpfsHash:'', cvDirectUrl:'', cvFileName:'CV.pdf' };
...
function triggerAdminLogin() {
  ...
  modal(
    ...
    '<div class="frow" style="text-align:left"><label class="flabel">Password</label>' +
      '<div style="position:relative"><input class="finput" id="auth-pw" type="password" placeholder="••••••••" onkeydown="if(event.key===\'Enter\')signInWithEmail()" style="padding-right:2.8rem">' +
      '<button onclick="togglePwVis()" style="position:absolute;right:.7rem;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--muted);font-size:.9rem;padding:0">👁</button></div></div>' +
...
function openPinataModal(){
  ...
  var hashInfo = latestHash
    ? '<div style="margin-top:.8rem;padding:.6rem .8rem;background:rgba(16,185,129,.06);border:1px solid rgba(16,185,129,.2);border-radius:6px;font-family:\'JetBrains Mono\',monospace;font-size:.65rem;color:#10b981">📌 Latest hash: '+latestHash.slice(0,28)+'…</div>'
    : '<div style="margin-top:.8rem;padding:.6rem .8rem;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:6px;font-family:\'JetBrains Mono\',monospace;font-size:.65rem;color:var(--muted)">No data saved to IPFS yet</div>';
  modal(
    '<div class="overlay" id="pin" onclick="oci(event,\'pin\')"><div class="mbox" style="max-width:440px">' +
    '<button class="mclose" onclick="cm(\'pin\')">&#10005;</button>' +
    '<div style="font-size:1.8rem;margin-bottom:.6rem">📌</div>' +
    '<div style="font-weight:600;color:#e6008a;margin-bottom:.4rem">Pinata Configuration</div>' +
    '<div style="font-size:.78rem;color:var(--muted);line-height:1.75;margin-bottom:1.1rem">Your JWT is saved permanently until you clear it.<br>Get it from <span style="color:#e6008a">app.pinata.cloud → API Keys</span>.</div>' +
    '<div class="frow"><label class="flabel">JWT Token</label>' +
      '<textarea class="finput" id="jwt-in" style="min-height:80px;font-size:.7rem;word-break:break-all;resize:vertical" placeholder="eyJhbGci...">'+pinataJWT+'</textarea></div>' +
    hashInfo +
    '<div style="display:flex;gap:.75rem;justify-content:flex-end;margin-top:1rem">' +
      '<button onclick="clearJWT()" style="padding:.6rem 1rem;background:transparent;border:1px solid rgba(248,113,113,.3);border-radius:6px;color:#f87171;font-size:.72rem;cursor:pointer">Clear JWT & Hash</button>' +
      '<button onclick="saveJWT()" style="padding:.6rem 1.5rem;background:#e6008a;border:none;border-radius:6px;color:#fff;font-size:.78rem;font-weight:700;cursor:pointer">Save</button>' +
    '</div>' +
    '</div></div>'
  );
}
...
function saveJWT(){
  ...
  if(!v){showToast('JWT cannot be empty',true);return;}
  pinataJWT=v;
...
function clearJWT(){
  if(!confirm('Clear JWT and latest hash? You will need to re-enter your JWT.'))return;
  pinataJWT=''; latestHash='';
  localStorage.removeItem('sg_pinata_jwt');
  localStorage.removeItem('sg_latest_hash');
  updatePinataLabel();
  cm('pin');
...
function buildLinksTab(){
  var cvStatus=profile.cvIpfsHash
    ...
    :(profile.cvDirectUrl?'<span style="color:#f97316;font-size:.72rem">✓ Direct URL set</span>':'<span style="color:var(--muted);font-size:.72rem">No CV uploaded</span>');
  return (
...
async function uploadCv(inp){
  ...
  try{
    var fd=new FormData();
    fd.append('file',file);
    fd.append('pinataMetadata',JSON.stringify({name:'sg-cv-'+Date.now()}));
    ...
    profile.cvDirectUrl='';
    applyProfileLinks();
    await saveAndSync('CV uploaded & saved to IPFS ✓');
    activeSect='links';
...
function showProjectForm(p){
  ...
  modal(
    '<div class="overlay" id="frm" onclick="oci(event,\'frm\')"><div class="mbox">' +
...
async function deleteItem(type,id){
  ...
  else if(type==='skills'){skills=skills.filter(function(x){return x.id!==id;});renderSkills();}
  else if(type==='stats'){stats=stats.filter(function(x){return x.id!==id;});renderStats();}
  else if(type==='experience'){experience=experience.filter(function(x){return x.id!==id;});renderExperience();}
  else if(type==='education'){education=education.filter(function(x){return x.id!==id;});renderEducation();}
  else if(type==='contact'){contact=contact.filter(function(x){return x.id!==id;});renderContact();}
...
async function uploadImg(inp){var file=inp.files[0];if(!file)return;document.getElementById('up-label').textContent='Uploading...';try{var fd=new FormData();fd.append('file',file);fd.append('pinataMetadata',JSON.stringify({name:'sg-img-'+Date.now()}));var res=await fetch(PINATA_API+'/pinning/pinFileToIPFS',{method:'POST',headers:{Authorization:'Bearer '+pinataJWT},body:fd});if(!res.ok)throw new Error(await res.text());var data=await res.json();currentImgHash=data.IpfsHash;document.getElementById('img-pre').innerHTML='<img src="'+PINATA_GATEWAY+data.IpfsHash+'" style="max-height:70px;border-radius:6px;margin-bottom:.4rem" alt="preview">';document.getElementById('up-label').textContent='📌 '+data.IpfsHash.slice(0,20)+'...';showToast('Image uploaded to IPFS ✓');}catch(e){showToast('Upload failed: '+e.message,true);document.getElementById('up-label').textContent='📤 Click to upload';}}
...
function renderProjects(){var grid=document.getElementById('projects-grid');if(!grid)return;var list=activeFilter==='all'?projects:projects.filter(function(p){return p.cat===activeFilter;});if(!list.length){grid.innerHTML='<div style="grid-column:1/-1;padding:4rem;text-align:center;color:var(--muted);font-size:.85rem">'+(projects.length?'No projects in this category':'No projects added yet')+'</div>';return;}grid.innerHTML=list.map(buildProjectCard).join('');reObserve();}
...
function renderExperience(){var el=document.getElementById('timeline');if(!el)return;if(!experience.length){el.innerHTML='<div style="color:var(--muted);font-size:.85rem;padding:2rem">No experience added yet</div>';return;}el.innerHTML=experience.map(function(e){return '<div class="tl-line"><div class="tl-dot"></div><div class="tl-date">'+esc(e.date)+'</div><div class="tl-title">'+esc(e.title)+'</div><div class="tl-org">'+esc(e.org)+'</div><p class="tl-desc">'+esc(e.desc)+'</p></div>';}).join('');}
...
function showToast(msg,err){var r=document.getElementById('toast-root');var d=document.createElement('div');d.className='toast'+(err?' err':'');d.textContent=msg;r.innerHTML='';r.appendChild(d);setTimeout(function(){d.remove();},2800);}
