...
(function loadFirebase() {
  ...
  scripts.forEach(function(src) {
    ...
    s.onload = function() {
      loaded++;
      if (loaded === scripts.length) onFirebaseReady();
    };
...
function onFirebaseReady() {
  ...
  // Expose auth to the rest of the file
  window._fbAuth = auth;
}
...
const DEFAULT_CONTACT = [
  { id:'c1', icon:'✉', label:'Email',    value:'krish.softtech228@gmail.com', href:'mailto:saturo@example.com' },
...
let profile = {};
let pinataJWT = '', activeFilter = 'all', editingId = null, techTags = [], currentImgHash = '', currentVideoHash = '';
let adminUnlocked = false;
let activeSect = 'projects';

function lsGet(k) { try { return JSON.parse(localStorage.getItem(k)); } catch { return null; } }
function lsSet(k, v) { localStorage.setItem(k, JSON.stringify(v)); }

function init() {
  stats      = lsGet('sg_stats')      || DEFAULT_STATS;
  skills     = lsGet('sg_skills')     || DEFAULT_SKILLS;
  projects   = lsGet('sg_projects')   || DEFAULT_PROJECTS;
  experience = lsGet('sg_experience') || DEFAULT_EXPERIENCE;
  education  = lsGet('sg_education')  || DEFAULT_EDUCATION;
  contact    = lsGet('sg_contact')    || DEFAULT_CONTACT;
  profile    = lsGet('sg_profile')    || Object.assign({}, DEFAULT_PROFILE);
  pinataJWT  = lsGet('sg_pinata_jwt') || '';
  updatePinataLabel();
  renderStats();
  renderSkills();
  renderProjects();
...
function applyProfileLinks() {
  var cvUrl = profile.cvIpfsHash
    ? (PINATA_GATEWAY + profile.cvIpfsHash)
    : (profile.cvDirectUrl || '#');

  document.querySelectorAll('[data-cv-btn], .btn-cv').forEach(function(el) {
    el.href = cvUrl;
    if (cvUrl !== '#') {
      el.setAttribute('download', profile.cvFileName || 'CV.pdf');
  ...
  document.querySelectorAll('[data-github-btn], .btn-github').forEach(function(el) {
    if (profile.githubUrl) el.href = profile.githubUrl;
  });

  document.querySelectorAll('[data-linkedin-btn], .btn-linkedin').forEach(function(el) {
    if (profile.linkedinUrl) el.href = profile.linkedinUrl;
  });

  document.querySelectorAll('.hero-actions .btn, #contact .btn, footer a').forEach(function(el) {
    var txt = el.textContent.trim().toLowerCase();
    if (txt.includes('download cv') || txt.includes('cv')) {
      el.href = cvUrl;
      if (cvUrl !== '#') el.setAttribute('download', profile.cvFileName || 'CV.pdf');
    }
...
function unlockAdmin() {
  ...
  var pill = document.getElementById('pinata-pill');
...
function triggerAdminLogin() {
  ...
  modal(
    ...
    '<div class="frow" style="text-align:left">' +
      ...
      '<input class="finput" id="auth-email" type="email" placeholder="admin@example.com" autocomplete="email">' +
    '</div>' +
    ...
    '<div class="frow" style="text-align:left">' +
      ...
      '<div style="position:relative">' +
        '<input class="finput" id="auth-pw" type="password" placeholder="••••••••" autocomplete="current-password" ' +
          'onkeydown="if(event.key===\'Enter\')signInWithEmail()" ' +
          'style="padding-right:2.8rem">' +
        '<button onclick="togglePwVis()" id="pw-toggle" title="Show/hide" ' +
          'style="position:absolute;right:.7rem;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--muted);font-size:.9rem;padding:0">👁</button>' +
      '</div>' +
    '</div>' +
    ...
    '<button onclick="signInWithEmail()" id="auth-submit" ' +
      'style="width:100%;padding:.75rem;background:var(--cyan);border:none;border-radius:8px;color:#000;font-weight:700;' +
      'font-family:\'JetBrains Mono\',monospace;font-size:.8rem;letter-spacing:1.5px;cursor:pointer;margin-bottom:.9rem;transition:opacity .2s">' +
      'SIGN IN' +
    '</button>' +
    ...
    '<div style="display:flex;align-items:center;gap:.7rem;margin-bottom:.9rem">' +
      ...
      '<span style="font-size:.65rem;color:var(--muted);font-family:\'JetBrains Mono\',monospace">OR</span>' +
      '<div style="flex:1;height:1px;background:var(--border)"></div>' +
    '</div>' +

    // Google sign-in button
    '<button onclick="signInWithGoogle()" ' +
      'style="width:100%;padding:.72rem;background:rgba(255,255,255,.05);border:1px solid var(--border);border-radius:8px;' +
      'color:var(--text);font-size:.8rem;font-weight:500;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.6rem;transition:background .2s">' +
...
function signInWithEmail() {
  if (!window._fbAuth) { showAuthError('Auth not ready, please wait…'); return; }
  ...
  window._fbAuth.signInWithEmailAndPassword(email, pw)
    ...
    .catch(function(err) {
      ...
      var msg = friendlyAuthError(err.code);
      showAuthError(msg);
...
function openAdmin() {
  ...
  var tabHTML = tabs.map(function(t) {
    var active = activeSect === t.key;
    return (
      '<button onclick="switchAdminTab(\'' + t.key + '\')" id="atab-' + t.key + '" style="padding:.42rem .9rem;border:1px solid ' +
      (active ? 'rgba(0,212,255,.5)' : 'var(--border)') +
      ';border-radius:100px;background:' + (active ? 'rgba(0,212,255,.1)' : 'transparent') +
      ';color:' + (active ? 'var(--cyan)' : 'var(--muted)') +
      ';font-size:.72rem;cursor:pointer;white-space:nowrap;transition:all .2s">' + t.icon + ' ' + t.label + '</button>'
    );
  }).join('');

  // Sign-out button in the top-right of admin panel
  var signOutBtn = '<button onclick="signOut()" title="Sign out" style="margin-left:auto;padding:.4rem .85rem;background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.25);border-radius:100px;color:#f87171;font-size:.7rem;cursor:pointer;white-space:nowrap;font-family:\'JetBrains Mono\',monospace">⏻ Sign out</button>';

  modal(
    '<div class="overlay" id="adm" onclick="oci(event,\'adm\')">' +
    '<div class="mbox" style="max-width:600px">' +
    '<button class="mclose" onclick="cm(\'adm\')">&#10005;</button>' +
    '<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin-bottom:1.4rem;align-items:center">' + tabHTML + signOutBtn + '</div>' +
    '<div id="admin-tab-content">' + getTabContent(activeSect) + '</div>' +
    '</div></div>'
  );
}
...
function buildLinksTab() {
  var cvStatus = profile.cvIpfsHash
    ...
    : (profile.cvDirectUrl
        ? '<span style="color:#f97316;font-size:.72rem;font-family:\'JetBrains Mono\',monospace">✓ Direct URL set</span>'
  ...
  return (
    ...
    '<div style="background:rgba(0,212,255,.03);border:1px solid rgba(0,212,255,.12);border-radius:10px;padding:1.1rem">' +
      '<div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.9rem">' +
        ...
        '<span style="font-size:.82rem;font-weight:600;color:var(--cyan)">CV / Resume</span>' +
        '<span style="margin-left:auto">' + cvStatus + '</span>' +
      '</div>' +
      '<div class="frow" style="margin-bottom:.7rem">' +
        '<label class="flabel">Upload CV (PDF) — stored on IPFS via Pinata</label>' +
      ...
      '<div class="frow" style="margin-bottom:.7rem">' +
        '<label class="flabel">Direct CV URL (Google Drive, Dropbox, etc.)</label>' +
        '<input class="finput" id="lnk-cv-url" value="' + esc(profile.cvDirectUrl || '') + '" placeholder="https://drive.google.com/file/d/...">' +
      '</div>' +
      '<div class="frow">' +
        '<label class="flabel">CV File Name</label>' +
        '<input class="finput" id="lnk-cv-name" value="' + esc(profile.cvFileName || 'CV.pdf') + '" placeholder="YourName_CV.pdf">' +
      '</div>' +
    '</div>' +
    '<div style="background:rgba(139,92,246,.03);border:1px solid rgba(139,92,246,.12);border-radius:10px;padding:1.1rem">' +
      '<div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.9rem">' +
        '<span style="font-size:1.15rem">🔗</span>' +
        '<span style="font-size:.82rem;font-weight:600;color:var(--purple)">Social Links</span>' +
      '</div>' +
...
function triggerCvUp() {
  if (!pinataJWT) { showToast('Set Pinata JWT first', true); return; }
  document.getElementById('cv-file-in').click();
}

async function uploadCv(inp) {
  var file = inp.files[0];
  if (!file) return;
  var lbl = document.getElementById('cv-up-label');
  if (lbl) lbl.textContent = 'Uploading…';
  try {
    var fd = new FormData();
    fd.append('file', file);
    fd.append('pinataMetadata', JSON.stringify({ name: file.name || 'CV.pdf' }));
    ...
    profile.cvIpfsHash  = data.IpfsHash;
    profile.cvFileName  = file.name || 'CV.pdf';
    profile.cvDirectUrl = '';
    lsSet('sg_profile', profile);
    applyProfileLinks();
    showToast('CV uploaded to IPFS ✓');
    activeSect = 'links';
...
function buildProjectsTab() {
  var rows = projects.map(function(p) {
    return (
      '<div style="display:flex;align-items:center;gap:.75rem;padding:.7rem;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:8px;margin-bottom:.5rem">' +
        '<span style="font-size:1.2rem">' + p.emoji + '</span>' +
        '<div style="flex:1;min-width:0">' +
          '<div style="font-size:.82rem;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(p.title) + '</div>' +
          '<div style="font-family:\'JetBrains Mono\',monospace;font-size:.6rem;color:var(--muted)">' + esc(p.num) + ' · ' + esc(p.cat) + '</div>' +
        '</div>' +
        '<button onclick="editProject(\'' + p.id + '\')" class="abtn-edit">Edit</button>' +
        '<button onclick="deleteItem(\'projects\',\'' + p.id + '\')" class="abtn-del">Del</button>' +
      '</div>'
    );
  }).join('');
...
function addProject() {
  editingId = null; techTags = []; currentImgHash = ''; currentVideoHash = '';
  var n = '#' + String(projects.length + 1).padStart(2, '0');
  showProjectForm({ id:'new_' + Date.now(), num:n, emoji:'🚀', title:'', desc:'', tech:[], cat:'web', github:'', videoHash:'', imageHash:'' });
}

function editProject(id) {
  var p = projects.find(function(x) { return x.id === id; });
  if (!p) return;
  editingId = id; techTags = p.tech.slice(); currentImgHash = p.imageHash || ''; currentVideoHash = p.videoHash || '';
  showProjectForm(p);
}
...
function showProjectForm(p) {
  ...
  var emojiBtns = emojis.map(function(e) {
    return '<button onclick="pickE(\'' + e + '\')" style="width:33px;height:33px;border:1px solid ' + (p.emoji === e ? 'var(--cyan)' : 'var(--border)') + ';border-radius:5px;background:' + (p.emoji === e ? 'rgba(0,212,255,.1)' : 'rgba(255,255,255,.03)') + ';font-size:1.05rem;cursor:pointer">' + e + '</button>';
  }).join('');
  ...
  modal(
    '<div class="overlay" id="frm" onclick="oci(event,\'frm\')">' +
    '<div class="mbox"><button class="mclose" onclick="openAdmin()">&#10005;</button>' +
...
function saveProjectForm() {
  ...
  lsSet('sg_projects', projects); renderProjects(); activeSect = 'projects'; openAdmin(); showToast('Project saved');
}

//  SKILLS TAB
function buildSkillsTab() {
  var rows = skills.map(function(sk) {
    return '<div style="display:flex;align-items:center;gap:.75rem;padding:.7rem;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:8px;margin-bottom:.5rem"><span style="font-size:1.2rem">' + sk.icon + '</span><div style="flex:1"><div style="font-size:.82rem;font-weight:500">' + esc(sk.title) + '</div><div style="font-size:.68rem;color:var(--muted);margin-top:.15rem">' + sk.tags.join(', ') + '</div></div><button onclick="editSkill(\'' + sk.id + '\')" class="abtn-edit">Edit</button><button onclick="deleteItem(\'skills\',\'' + sk.id + '\')" class="abtn-del">Del</button></div>';
  }).join('');
...
function saveExpForm() {
  ...
  if (editingId) { experience = experience.map(function(x) { return x.id === editingId ? e : x; }); } else { experience.push(e); }
  lsSet('sg_experience', experience); renderExperience(); activeSect = 'experience'; openAdmin(); showToast('Experience saved');
}
...
function buildEducationTab() {
  var rows = education.map(function(ed) {
    return '<div style="display:flex;align-items:center;gap:.75rem;padding:.7rem;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:8px;margin-bottom:.5rem"><span style="font-size:1.2rem">' + ed.badgeEmoji + '</span><div style="flex:1"><div style="font-size:.82rem;font-weight:500">' + esc(ed.deg) + '</div><div style="font-size:.68rem;color:var(--muted)">' + esc(ed.school) + ' · ' + esc(ed.year) + '</div></div><button onclick="editEdu(\'' + ed.id + '\')" class="abtn-edit">Edit</button><button onclick="deleteItem(\'education\',\'' + ed.id + '\')" class="abtn-del">Del</button></div>';
  }).join('');
...
function saveEduForm() {
  ...
  if (editingId) { education = education.map(function(x) { return x.id === editingId ? ed : x; }); } else { education.push(ed); }
  lsSet('sg_education', education); renderEducation(); activeSect = 'education'; openAdmin(); showToast('Education saved');
}

//  CONTACT TAB
function buildContactTab() {
  var rows = contact.map(function(c) {
    return '<div style="display:flex;align-items:center;gap:.75rem;padding:.7rem;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:8px;margin-bottom:.5rem"><span style="font-size:1.2rem">' + c.icon + '</span><div style="flex:1"><div style="font-size:.82rem;font-weight:500">' + esc(c.label) + '</div><div style="font-size:.68rem;color:var(--muted)">' + esc(c.value) + '</div></div><button onclick="editContact(\'' + c.id + '\')" class="abtn-edit">Edit</button><button onclick="deleteItem(\'contact\',\'' + c.id + '\')" class="abtn-del">Del</button></div>';
  }).join('');
...
function saveContactForm() {
  ...
  if (editingId) { contact = contact.map(function(x) { return x.id === editingId ? c : x; }); } else { contact.push(c); }
  lsSet('sg_contact', contact); renderContact(); activeSect = 'contact'; openAdmin(); showToast('Contact saved');
}
...
function deleteItem(type, id) {
  ...
  var store = map[type];
  if (!store || !confirm('Remove this item?')) return;
  map[type] = store.filter(function(x) { return x.id !== id; });
  if      (type === 'projects')  { projects   = map[type]; lsSet('sg_projects',   projects);   renderProjects();   }
  else if (type === 'skills')    { skills     = map[type]; lsSet('sg_skills',     skills);     renderSkills();     }
  else if (type === 'experience'){ experience = map[type]; lsSet('sg_experience', experience); renderExperience(); }
  else if (type === 'education') { education  = map[type]; lsSet('sg_education',  education);  renderEducation();  }
  else if (type === 'contact')   { contact    = map[type]; lsSet('sg_contact',    contact);    renderContact();    }
...
function pickE(e) {
  ...
  document.querySelectorAll('#epicker button').forEach(function(b) { var s = b.textContent === e; b.style.border = '1px solid ' + (s ? 'var(--cyan)' : 'var(--border)'); b.style.background = s ? 'rgba(0,212,255,.1)' : 'rgba(255,255,255,.03)'; });
}

function tagEl(t) {
  return '<span style="padding:.2rem .6rem;background:rgba(0,212,255,.06);border:1px solid rgba(0,212,255,.2);border-radius:5px;font-size:.7rem;color:var(--cyan);font-family:\'JetBrains Mono\',monospace;display:inline-flex;align-items:center;gap:.3rem">' + esc(t) + '<span onclick="rmTag(\'' + esc(t) + '\')" style="cursor:pointer;color:#f87171">&#215;</span></span>';
}

function addTag() {
  var inp = document.getElementById('f-tech-in');
  var val = inp.value.trim();
  if (val && !techTags.includes(val)) { techTags.push(val); document.getElementById('tags-out').innerHTML = techTags.map(tagEl).join(''); }
  inp.value = '';
}
...
async function uploadImg(inp) {
  ...
  try {
    var fd = new FormData(); fd.append('file', file); fd.append('pinataMetadata', JSON.stringify({ name: document.getElementById('f-title').value || 'img' }));
    ...
    document.getElementById('up-label').textContent = '📌 ' + data.IpfsHash.slice(0, 20) + '...'; showToast('Uploaded to IPFS');
...
function triggerVideoUp() {
  if (!pinataJWT) { showToast('Set Pinata JWT first', true); return; }
  document.getElementById('vid-in').click();
}
...
async function uploadVideo(inp) {
  ...
  try {
    var fd = new FormData(); fd.append('file', file); fd.append('pinataMetadata', JSON.stringify({ name: (document.getElementById('f-title').value || 'video') + '-demo' }));
    var res = await fetch('https://api.pinata.cloud/pinning/pinFileToIPFS', { method:'POST', headers:{ Authorization:'Bearer ' + pinataJWT }, body:fd });
    if (!res.ok) throw new Error(await res.text());
...
function saveJWT() {
  var v = document.getElementById('jwt-in').value.trim(); pinataJWT = v; lsSet('sg_pinata_jwt', v); updatePinataLabel(); cm('pin'); showToast(v ? 'Pinata JWT saved' : 'JWT cleared');
}
...
function renderProjects() {
  ...
  if (!list.length) { grid.innerHTML = '<div style="grid-column:1/-1;padding:4rem;text-align:center;color:var(--muted);font-size:.85rem">No projects in this category</div>'; return; }
  grid.innerHTML = list.map(buildProjectCard).join(''); reObserve();
}
...
function buildProjectCard(p) {
  ...
  return '<div class="project-card reveal" onclick="openProjectViewer(\'' + p.videoHash + '\')"><div class="project-img">' + mediaHtml + '<div class="project-num">' + esc(p.num) + '</div><div class="project-cat-pill" style="background:' + c.bg + ';border:1px solid ' + c.border + ';color:' + c.text + '">' + esc(p.cat) + '</div></div><div class="project-body"><div class="project-title">' + esc(p.title) + '</div><div class="project-desc">' + esc(p.desc) + '</div><div class="project-tech">' + p.tech.map(function(t) { return '<span class="ptag">' + esc(t) + '</span>'; }).join('') + '</div><div class="project-links">' + (p.github && p.github !== '#' ? '<a href="' + p.github + '" class="plink" target="_blank">&#8599; GitHub</a>' : '') + '</div></div></div>';
}

function openProjectViewer(videoHash) {
  if (!videoHash) { showToast('No video available', true); return; }
  modal('<div class="overlay video-modal" id="vidModal" onclick="oci(event,\'vidModal\')"><div class="video-box"><button class="video-close" onclick="cm(\'vidModal\')">&#10005;</button><video src="' + PINATA_GATEWAY + videoHash + '" controls autoplay></video></div></div>');
}

function filter(cat, btn) {
  activeFilter = cat;
  document.querySelectorAll('.fbtn').forEach(function(b) { b.classList.remove('active'); });
  btn.classList.add('active'); renderProjects();
}
...
function renderEducation() {
  ...
  el.innerHTML = education.map(function(ed) {
    ...
    var bs = BADGE_STYLES[ed.badgeColor] || BADGE_STYLES.cyan;
...
function renderContact() {
  ...
  el.innerHTML = contact.map(function(c) { return '<a href="' + c.href + '" class="contact-card" ' + (c.href.startsWith('http') ? 'target="_blank"' : '') + '><span class="cc-icon">' + c.icon + '</span><span class="cc-label">' + esc(c.label) + '</span><span class="cc-val">' + esc(c.value) + '</span></a>'; }).join('');
}
...
function startTypewriter() {
  ...
  function buildLineHTML(line, charCount) {
    switch (line.type) {
      ...
      case 'prop': {
        var raw = '  ' + line.key + ': ' + line.val + ','; var ch = charCount; var html = '';
        var parts = [{ t:'  ' }, { t:line.key, cls:'t-var' }, { t:': ' }];
        if (line.isArr) { parts = parts.concat([{ t:'[' }, { t:'"Full-Stack"', cls:'t-str' }, { t:', ' }, { t:'"Blockchain"', cls:'t-str' }, { t:']' }, { t:',' }]); } else { parts = parts.concat([{ t:line.val, cls:line.valClass }, { t:',' }]); }
      ...
      case 'method': {
        var raw = '  build: () => "future"'; var ch = charCount; var html = '';
...
function showToast(msg, err) {
  ...
  d.textContent = msg; r.innerHTML = ''; r.appendChild(d);
...
var obs = new IntersectionObserver(function(entries) {
  entries.forEach(function(e, i) { if (e.isIntersecting) setTimeout(function() { e.target.classList.add('in'); }, i * 55); });
}, { threshold:.08 });
